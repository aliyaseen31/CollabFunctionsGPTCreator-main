...
def get_hashed_filename(func_name, *args, **kwargs):
    m = hashlib.md5()
    data = f"{func_name}_{str(args)}_{str(kwargs)}".encode()
    m.update(data)
...
def save_to_pickle(func):
    ...
    def wrapper(*args, **kwargs):
        # print(f"[save_to_pickle] func:{func.__name__} args: {args}, kwargs: {kwargs}")
        result = func(*args, **kwargs)
        if not PickleCacheActivated:
            return result
        ...
        # save all the arguments in clear to another file
        with open(filename+".content", 'w') as file:
...
def load_from_pickle(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # print(f"[load_from_pickle] func:{func.__name__} args: {args}, kwargs: {kwargs}")
        filename = get_hashed_filename(func.__name__, *args, **kwargs)
        # update filename given pkl files are in pkl_cache folder
        filename = os.path.join("pkl_cache", filename)
        if os.path.exists(filename):
            try:
                with open(filename, 'rb') as file:
                    #print(f"Loading from pickle: {filename}")
                    return pickle.load(file)
            except Exception as e:
                # This will catch any exception while loading the pickle, 
                # such as a corrupted file or incompatible data
                print(f"Error loading from pickle: {e}")

        # Call the original function if the file doesn't exist or there's an error
        result = func(*args, **kwargs)
        return result
    return wrapper

def create_callable_function_with_args(func, *args, **kwargs):
    def callable_with_args():
        return func(*args, **kwargs)
    return callable_with_args
